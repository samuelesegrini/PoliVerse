...
/// welcome.
///
/// The first run used to be five symbols over five paragraphs, which is a
/// brochure: it said what the app does without ever showing it. This shows the
/// screens themselves — the same navigation bar, the same tab bar, the same
/// cards in the material the look is set to.
///
/// ## The movement
///
...
/// 2. It **settles**: the scale eases back to life size, the wall drops into
///    place and the blur resolves, all on one curve.
...
///    that takes its colour from whatever card is in the middle, blended
///    with the next one's in the proportion the row is between them, so it slides
...
///
/// There is no second copy of where the row is. The scroll view's own
/// `ScrollPosition` is what the drift writes and what a finger writes, and
/// `onScrollGeometryChange` reports where the row actually is, in points and
/// in cards, on every frame of both. The ground is coloured from that, and
/// each leg of the drift starts from it, which is why a drag and the drift
/// behave identically and neither can leave the other behind.
///
/// Nothing snaps. The drift asks for an offset rather than a card, because a
...
struct FeatureTour<Actions: View>: View {
    /// Everything the movement is made of, in one place.
    struct Feel {
        ...
        /// What a card away from the middle shrinks to, and fades to.
        var sideScale: CGFloat = 0.9
        var sideOpacity: CGFloat = 0.82
        /// How far a card away from the middle turns, in degrees.
        var sideRotation: Double = 15
        ...
        /// How long the wall takes to arrive, and where it arrives from: a
        /// scale, a lift, and a blur that all resolve together.
        var settle: Duration = .seconds(1.15)
        ...
        /// How far one leg of the drift carries before the next is asked for.
        ///
        /// Long, because each leg is a single animation: the fewer joins
        /// there are, the fewer chances of a seam in a movement whose whole
        /// point is not to have any. At this pace one leg lasts a minute and
        /// a half, far longer than this screen is usually looked at.
        var legCards: CGFloat = 24
        /// How long the row keeps still after a touch before moving on again.
        var restAfterTouch: Duration = .seconds(1.8)
        /// Copies of the deck laid end to end, so the row has no end to reach
        /// in any time someone spends on this screen. Lazy, so only the cards
        /// in view are ever built.
        ///
        /// Forty, not two hundred: starting in the middle leaves a hundred
        /// cards of runway forward, which at this pace is six minutes of
        /// drift — far longer than a welcome screen is ever looked at — and
        /// it asks the scroll view to lay out a fifth as much content.
        var laps = 40

        /// The words, once the wall is nearly home.
        var wordsBegin: Double = 0.55
        var wordsStagger: Double = 0.09
        var wordBlur: CGFloat = 12
    }
    ...
    /// number kept beside it: the same value a finger writes and the loop
    ...
    /// finger and under the drift alike.
    @State private var drift = Drift()
    /// One card and the gap after it: what a card's worth of drift measures.
    @State private var stride: CGFloat = 0
    /// Whether the row has been put on its starting card yet.
    @State private var placed = false
    /// Whether a finger is on the row, or it is still coasting from one.
    ...
    var body: some View {
        GeometryReader { proxy in
            ...
            ZStack(alignment: .top) {
                TourBackdrop(accent: middleAccent(at: drift.cards))
                    .ignoresSafeArea()
                ...
                VStack(spacing: 0) {
                    ...
                    Spacer(minLength: 0)
                    actions
                        .modifier(BlurReveal(progress: wordProgress(lastPiece + 1), blur: feel.wordBlur))
                }
            }
            .frame(width: size.width, height: size.height)
        }
        .task { await arrive() }
        // Restarted whenever a finger lands or lets go: touching cancels the
        // loop, letting go starts a fresh one after a rest.
        .task(id: isTouched) { await advance() }
    }
    ...
    private func wall(size: CGSize, cardWidth: CGFloat, cardHeight: CGFloat) -> some View {
            ...
            LazyHStack(spacing: feel.spacing) {
                ForEach(0..<screens.count * feel.laps, id: \.self) { index in
                    TourCard(screen: screens[index % screens.count], size: CGSize(width: cardWidth, height: cardHeight))
                        ...
                        // The cards either side shrink, fade and turn away,
                        // measured off the scroll view itself.
                        .scrollTransition(.interactive, axis: .horizontal) { content, phase in
                            let away = min(abs(phase.value), 1)
                            return content
                                .scaleEffect(1 - (1 - feel.sideScale) * away)
                                .opacity(1 - (1 - feel.sideOpacity) * away)
                                .rotation3DEffect(.degrees(phase.value * feel.sideRotation),
        ...
        // Where the row is, in cards, on every frame it moves — under a
        // finger and under the loop alike. This is what the ground follows,
        // which is why its colour slides between two cards instead of
        // snapping when one arrives.
        .onScrollGeometryChange(for: Drift.self) { geometry in
            let stride = cardWidth + feel.spacing
            guard stride > 0 else { return Drift() }
            return Drift(offset: geometry.contentOffset.x,
                         cards: (geometry.contentOffset.x + geometry.contentInsets.leading) / stride)
        } action: { _, moved in
            drift = moved
        }
        .onScrollPhaseChange { _, phase in
            // The row's own glide to the next card is `.animating`: only a
            // finger, or the coast after one, counts as a touch.
            isTouched = phase == .tracking || phase == .interacting || phase == .decelerating
        }
        .onAppear {
            stride = cardWidth + feel.spacing
            guard !placed else { return }
            placed = true
            position.scrollTo(id: start, anchor: .center)
            drift.cards = CGFloat(start)
        }
        // The arrival: one curve carrying scale, lift and focus together.
        .scaleEffect(feel.fromScale + (1 - feel.fromScale) * arrival, anchor: .top)
        .offset(y: -feel.fromLift * (1 - arrival))
        .blur(radius: feel.fromBlur * (1 - arrival))
    ...
    private func middleAccent(at cards: CGFloat) -> Color {
        ...
        let crossed = Double(cards - CGFloat(leaving))
    ...
    private var words: some View {
        VStack(spacing: 6) {
            Text(overline)
                ...
                .modifier(BlurReveal(progress: wordProgress(0), blur: feel.wordBlur))
            ...
            HStack(spacing: 8) {
                ForEach(Array(title.split(separator: " ").enumerated()), id: \.offset) { index, word in
                    Text(String(word))
                        .modifier(BlurReveal(progress: wordProgress(index + 1), blur: feel.wordBlur))
                }
            }
            ...
            Text(detail)
                ...
                .padding(.top, 6)
                .modifier(BlurReveal(progress: wordProgress(lastPiece), blur: feel.wordBlur))
        }
        .multilineTextAlignment(.center)
    ...
    /// How far into its own reveal one piece of the caption is.
    private func wordProgress(_ index: Int) -> CGFloat {
        let begin = feel.wordsBegin + Double(index) * feel.wordsStagger
        let span = max(1 - begin, 0.01)
        return CGFloat(min(max((Double(arrival) - begin) / span, 0), 1))
    }
    ...
    private func arrive() async {
        ...
        // Eased out rather than sprung: the reference settles without
        // overshooting, which a bounce would give it.
        withAnimation(.easeOut(duration: feel.settle.seconds)) { arrival = 1 }
    }



    /// Keeps the row moving, evenly, for as long as nobody is touching it.
    private func advance() async {
        guard !isTouched, !reduceMotion else { return }
        // The wall arrives before it drifts; after a touch, the row rests a
        // moment before taking over again.
        try? await Task.sleep(for: arrival < 1 ? feel.settle : feel.restAfterTouch)

        // Straight to pace, with no ease-in. An ease-in from a standstill
        // spends its first second moving by less than a point per frame,
        // which does not look like a wall getting going — it looks like a
        // wall dropping frames. At eighty points a second there is nothing to
        // ease into: starting at the pace is itself gentle.
        while !Task.isCancelled {
            let seconds = Double(feel.legCards) * feel.secondsPerCard
            await glide(cards: feel.legCards, over: .seconds(seconds), on: .linear(duration: seconds))
        }
    }

    /// Carries the row on by a distance in cards, and waits out the journey.
    ///
    /// Asked of the scroll view by offset rather than by card, because a
    /// drift stops between cards as readily as on one, and an id can only
    /// name a whole card.
    private func glide(cards distance: CGFloat, over duration: Duration, on curve: Animation) async {
        guard !Task.isCancelled, stride > 0, distance > 0 else { return }
        withAnimation(curve) {
            position.scrollTo(x: drift.offset + distance * stride)
        }
...
/// Where the row is, as the scroll view last reported it: in points, which
/// is what the next leg of the drift is asked for, and in cards, which is
/// what the ground behind the wall is coloured from.
...
private struct BlurReveal: ViewModifier {
    let progress: CGFloat
    let blur: CGFloat

    func body(content: Content) -> some View {
        content
            .blur(radius: blur * (1 - progress))
    ...
    var body: some View {
        ZStack {
            ...
            RadialGradient(colors: [accent.opacity(0.55), .clear],
                           center: .init(x: 0.2, y: 0.18), startRadius: 0, endRadius: 420)
            RadialGradient(colors: [accent.opacity(0.4), .clear],
                           center: .init(x: 0.9, y: 0.32), startRadius: 0, endRadius: 380)
            RadialGradient(colors: [accent.opacity(0.28), .clear],
                           center: .init(x: 0.5, y: 0.78), startRadius: 0, endRadius: 520)
        }
        // No blur pass, and no animation.
        //
        // Both were there to hide a step that no longer happens. The colour
        // used to arrive a card at a time, so it needed softening and a curve
        // to slide along; it now arrives a frame at a time off the scroll
        // itself. What is left is three radial gradients, which are already
        // as soft as a blur would make them — blurring them cost a
        // full-screen offscreen pass every frame to change almost nothing,
        // and animating a value that moves every frame started a new
        // animation every frame to chase a number that had already arrived.
