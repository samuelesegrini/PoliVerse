...
- **[V] The day stepper outruns the data.** It spans ±60 days (`DayStrip.swift:18-21`). NewUI loads the agenda only with `load(around: .now)` (`NewRootView.swift:99`), which covers −7 days to +1 month (`PoliVerse/Model/Timetable/AgendaService.swift:70-71,170`). Nothing reloads when `shell.day` changes, so later days show an empty Oggi.
...
- Whether career and deadline data for Oggi's sections arrives at launch. NewUI loads only the agenda. `FreshnessCoordinator.revalidate` runs every load when the app becomes active (`PoliVerse/Model/Sync/FreshnessCoordinator.swift:94-117`), which probably covers it.
